...
export default function SettingsPage() {
  ...
  async function createTeam() {
    ...
    setTeams(t => [...t, { id: ref.id, name: teamName.trim() }])
  ...
  async function createART() {
    ...
    setArts(a => [...a, { id: ref.id, name: artName.trim() }])
    ...
    <div className="p-8 max-w-2xl">
      ...
      <section className="mb-10">
        ...
        {teams.length > 0 && (
          <div className="space-y-2 mb-4">
            {teams.map(t => (
              <div key={t.id} className="flex items-center justify-between p-3 bg-white
      ...
      <section>
        ...
        {arts.length > 0 && (
          <div className="space-y-2 mb-4">
            {arts.map(a => (
              <div key={a.id} className="flex items-center justify-between p-3 bg-white
                             rounded-xl border border-ink-100">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-xl border border-ink-200 text-sm
                       focus:outline-none focus:ring-2 focus:ring-brand-400 focus:border-transparent"
          />
          <button onClick={createART} disabled={saving || !artName.trim()}
            className="px-4 py-2.5 bg-brand-500 text-white rounded-xl text-sm font-medium
                       hover:bg-brand-600 transition-colors disabled:opacity-40">
            Create
          </button>
